...
func FromEnv() (Config, error) {
	c := Config{
		...
		MinTTL:            getenvDuration("MIN_TTL", 3*time.Minute),
		MaxTTL:            getenvDuration("MAX_TTL", 5*time.Hour+18*time.Minute),
		DefaultTTL:        getenvDuration("DEFAULT_TTL", 18*time.Minute),
