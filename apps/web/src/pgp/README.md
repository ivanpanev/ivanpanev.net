...
site address. Until the file exists *and is untracked*, `/pgp` and `/verify`
render a "not yet published" state and the build logs a warning. After this
file is committed, a missing key fails the build. `IVP_REQUIRE_WKD=1` forces
that fail even before the ceremony.
