...
| M7 operator actions | M7 | - | (a) `cert-manager/wildcard-ivanpanev-net` DNS-01 is stuck: the token in `cloudflare-api-token.secret.yaml` is the operator's account token, IP-locked to the operator's home `/32`, so cert-manager on the Hetzner nodes gets `9109 Cannot use the access token from location`. Create a dedicated token (Zone DNS Edit + Zone Read on `ivanpanev.net`, no IP condition), re-encrypt the secret with sops, `kubectl -n cert-manager delete challenge --all` to retrigger. (b) Authentik `/if/flow/initial-setup/` behind Access (carried from M6). (c) First Barman backup then `scripts/restore-drill.sh` (carried from M5). (d) `web.yml` `deploy to production` fails at `wrangler deploy` with `9109 Cannot use the access token from location: <runner ip>` for the same reason as (a): `CLOUDFLARE_API_TOKEN` in the GitHub `production`/`preview` environments is the IP-locked operator token. The live site was deployed from the operator's machine. Mint a deploy token (Workers Scripts Edit, Workers Routes Edit, Account Settings Read, Zone Read on `ivanpanev.net`, no IP condition; GitHub runner IPs rotate) and replace both environment secrets, then re-run the latest `web.yml`. | Open (operator) |
