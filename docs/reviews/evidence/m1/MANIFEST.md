...
| --- | --- | --- |
| `astro-check.txt` | `pnpm check` | `Result (67 files): 0 errors, 0 warnings, 0 hints`, exit 0 |
| `unit-tests.txt` | `pnpm test -- --coverage` | 124 tests pass in 8 files; coverage above the `vitest.config.ts` thresholds |
...
| `e2e.txt` | `pnpm exec playwright test --reporter=list` | previous pack: 115 passed against `wrangler dev`. Round-2 change is headers + check-dist only; full e2e not re-run. Local workerd does not apply Host-based `_redirects` (GET `/` with `Host: www.ivanpanev.net` returns 200). The rule itself is asserted by `check-dist.mjs`. |
...
| Custom-domain routing (`ivanpanev.net`, `www`) | first `wrangler deploy` attaches the domains; local workerd ignores Host-based `_redirects` | operator: `curl -sI -H 'Host: www.ivanpanev.net' https://www.ivanpanev.net/` must be 301 to `https://ivanpanev.net/` |
